...
def main_menu(name):
    ...
    btn_recovery = types.InlineKeyboardButton("🔑 Відновити пароль", url="https://www.python.org/downloads/")
    btn_site = types.InlineKeyboardButton("🌐 Сайт", url="https://www.python.org/downloads/")
    btn_forum = types.InlineKeyboardButton("📔 Форум", url="https://www.python.org/downloads/")
    btn_shop = types.InlineKeyboardButton("🏪 Магазин", url="https://www.python.org/downloads/")
    ...
    try:
        if call.data == "support":
            ...
            if ticket_id in ticket_messages:
                ...
                new_caption = old_caption.split("Статус:")[0] + f"Статус: Оператор {operator_name} підключився."
                bot.edit_message_caption(chat_id=chat_id_from_ticket,
                                         message_id=message_id_from_ticket,
                                         caption=new_caption,
                                         reply_markup=None,
                                         parse_mode="HTML")
            bot.send_message(user_id, f"Оператор {operator_name} підключився до вашого тікету.")
            active_chats[ticket_id] = (operator_id, user_id)
            if ticket_id in pending_tickets:
                del pending_tickets[ticket_id]
            log_message(ticket_id, f"Оператор {operator_name} прийняв тікет")
            ...
            if ticket_id in active_chats:
                ...
                bot.send_message(user_id, "Тікет закрито оператором. Оцініть роботу:", reply_markup=markup)
                del active_chats[ticket_id]
        ...
        if message.from_user.id in OPERATORS:
            for ticket_id, (operator_id, user_id) in active_chats.items():
                if operator_id == message.from_user.id:
                    ...
                    return
            bot.send_message(message.from_user.id, "❌ Немає активних тікетів")
        else:
            for ticket_id, (operator_id, user_id) in active_chats.items():
                if user_id == message.from_user.id:
                    operator_name = OPERATORS.get(operator_id, "Оператор")
                    bot.send_message(operator_id, f"Повідомлення від користувача (тікет #{ticket_id}): {message.text}")
                    log_message(ticket_id, f"Повідомлення від користувача: {message.text}")
                    return
            bot.send_message(message.chat.id, "Я не розумію вас. Спробуйте /start")
    except Exception as e:
