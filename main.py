...
OPERATOR_ID = 8345368586

bot = telebot.TeleBot(TOKEN)

ticket_counter = 0
OPERATORS = {OPERATOR_ID: "𝕄𝕒𝕜𝕤𝕚𝕞𝕜𝕒 𝕄𝕒𝕝𝕚𝕟𝕠𝕧𝕤𝕜𝕪"}
active_chats = {}
ticket_messages = {}
...
def main_menu(name):
    ...
    btn_recovery = types.InlineKeyboardButton("Сайт", url="https://white-aist.netlify.app/")
    btn_site = types.InlineKeyboardButton("Discord", url="https://discord.gg/GwECwquJTd")
    btn_forum = types.InlineKeyboardButton("В разработке", url="https://www.python.org/downloads/")
    btn_shop = types.InlineKeyboardButton("В разработке", url="https://www.python.org/downloads/")
    ...
    try:
        for operator_id in OPERATORS.keys():
            ...
            bot.send_message(operator_id,
                             f"🔔 Новий тикет #{ticket_id}\nID користувача: {user_id}\nОт: @{username}",
                             reply_markup=markup)
        log_message(ticket_id, f"Запрос отправлено оператором от @{username}")
    except Exception as e:
...
def callback_query(call):
    ...
    try:
        if call.data == "support":
            markup = types.InlineKeyboardMarkup()
            btn_connect = types.InlineKeyboardButton("Подлючить оператора", callback_data="connect_operator")
            btn_back = types.InlineKeyboardButton("⬅️ Назад", callback_data="back_to_start")
            markup.add(btn_connect, btn_back)
            text = f"Вітаю, {username}!\n\nНатисніть 'Подлючить оператора' для запроса."
            ...
            text = f"Вітаю, {username}!\n\nНомер тикета: #{ticket_counter}\nСтатус: <i>Ожыдание оператора...</i>"
            bot.edit_message_caption(chat_id=call.message.chat.id,
                                     message_id=call.message.message_id,
                                     caption=text,
                                     reply_markup=markup,
            ...
            if ticket_id in ticket_messages:
                ...
                caption = old_caption.split("Статус:")[0] + "Статус: Тикет отменено."
                markup = types.InlineKeyboardMarkup()
                btn_back = types.InlineKeyboardButton("⬅️ Назад", callback_data="back_to_start")
                markup.add(btn_back)
                ...
                log_message(ticket_id, f"Тикет #{ticket_id} отменено @{username}")

        elif call.data.startswith("accept_"):
            parts = call.data.split("_")
            ticket_id = int(parts[1])
            if ticket_id in canceled_tickets:
                bot.answer_callback_query(call.id, text="Тикет отменено")
                return
            user_id = int(parts[2])
            ...
            if ticket_id in ticket_messages:
                ...
                new_caption = old_caption.split("Статус:")[0] + f"Статус: Оператор {operator_name} подлючился."
                bot.edit_message_caption(chat_id=chat_id_from_ticket,
                                         message_id=message_id_from_ticket,
                                         caption=new_caption,
                                         reply_markup=None,
                                         parse_mode="HTML")
            bot.send_message(user_id, f"Оператор {operator_name} Подключился за вашим запросом.")
            active_chats[ticket_id] = (operator_id, user_id)
            if ticket_id in pending_tickets:
                del pending_tickets[ticket_id]
            log_message(ticket_id, f"Оператор {operator_name} принял тикет")
            bot.answer_callback_query(call.id, text="Тикет прийнято")
            ...
            if ticket_id in active_chats:
                ...
                bot.send_message(user_id, "Тикет закрыто оператором. Оцените работу:", reply_markup=markup)
                del active_chats[ticket_id]
            ...
            ticket_id = int(parts[1])
            rating = int(parts[2])
            log_message(ticket_id, f"Оцінка оператора: {rating}/5")
            bot.send_message(call.from_user.id, "Благодарим за оценку!")
            bot.answer_callback_query(call.id, text="Оценка принята!")
        ...
        bot.answer_callback_query(call.id, text="Слyчилась ошыбка")
        ...
        if message.from_user.id in OPERATORS:
            for ticket_id, (operator_id, user_id) in active_chats.items():
                if operator_id == message.from_user.id:
                    ...
                    log_message(ticket_id, f"Сообщение от оператора {OPERATORS[message.from_user.id]}: {message.text}")
                    return
            bot.send_message(message.from_user.id, "❌ Нет активних тикетов")
        else:
            for ticket_id, (operator_id, user_id) in active_chats.items():
                if user_id == message.from_user.id:
                    operator_name = OPERATORS.get(operator_id, "Оператор")
                    bot.send_message(operator_id, f"Сообщение от пользователя (тікет #{ticket_id}): {message.text}")
                    log_message(ticket_id, f"Сообщение от пользователя: {message.text}")
                    return
            bot.send_message(message.chat.id, "Ты перебухал? Юзни /start")
    except Exception as e:
